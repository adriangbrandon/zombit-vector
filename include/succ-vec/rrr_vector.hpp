...
class select_support_rrr
{
    ...
    public:
        ...
        /*size_type succ(size_type i) const
        {
            ...
            if(bt == 0){
                ++bt_idx;
                if(bt_idx == m_v->bt.size()) return m_v->size();
                //(1) Look into the current super-block
                //sample_pos = bt_idx/t_k;
                //size_type cnt_sb = 0, rank_sb = m_v->m_rank[sample_pos-1];
                bool inv =  m_v->m_invert[sample_pos];
                size_type btnrp = m_v->m_btnrp[ sample_pos ];
                for (size_type j = sample_pos*t_k; j < bt_idx; ++j) {
                    bt = inv ? t_bs - m_v->m_bt[j] :  m_v->m_bt[j];
                    btnrp += rrr_helper_type::space_for_bt(bt);
                }

                uint16_t btnrlen;
                while (bt_idx < m_v->bt.size()) {
                    sample_pos = bt_idx/t_k;
                    bt = m_v->m_invert[sample_pos] ? t_bs-m_v->m_bt[bt_idx] : m_v->m_bt[bt_idx];
                    btnrlen = rrr_helper_type::space_for_bt(bt);
                    if(bt > 0) break;
                    btnrp += btnrlen;
                    ++bt_idx;
            ...
            uint16_t btnrlen = rrr_helper_type::space_for_bt(bt);
            number_type btnr = rrr_helper_type::decode_btnr(m_v->m_btnr, btnrp, btnrlen);
            //std::cout << rrr_helper_type::decode_int(bt, btnr, 0, btnrlen) << std::endl;
            uint64_t data = rrr_helper_type::decode_int(bt, btnr, off, t_bs-off);
            if(!data){
                //(1) Look for the next block with 1s in the current super-block
                off = 0;
                ++bt_idx;
                btnrp += btnrlen;
                //TODO: smaller than the end of current super-block (min(sample_pos + 1*t_k, m_v->bt.size()))
                //uint64_t last = std::min(sample_pos + 1*t_k, m_v->bt.size());
                while (bt_idx < m_v->bt.size()) {
                    sample_pos = bt_idx/t_k;
                    bt = m_v->m_invert[sample_pos] ? t_bs-m_v->m_bt[bt_idx] : m_v->m_bt[bt_idx];
                    btnrlen = rrr_helper_type::space_for_bt(bt);
                ...
                //TODO: (2) Exponential search looking for the next super-block with 1s
                if(!bt){
                ...
                btnr = rrr_helper_type::decode_btnr(m_v->m_btnr, btnrp, btnrlen);
                data = rrr_helper_type::decode_int(bt, btnr, off, t_bs-off);
            }
        ...
        size_type succ(size_type i) const
        {
            ...
            number_type btnr = rrr_helper_type::decode_btnr(m_v->m_btnr, btnrp, btnrlen);
            uint64_t data = rrr_helper_type::decode_int(bt, btnr, off, t_bs-off);
            if(!data){ //(3) Search in a different block
