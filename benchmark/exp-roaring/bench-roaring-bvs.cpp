...
void run(const std::string &path, const std::string &type, const std::vector<uint64_t> &qrandom){
    ...
    std::cerr << bytes << ";" << avg_member << ";" << avg_sequential << ";" << avg_intersection
    << ";" << avg_sequential_iter << ";" << avg_intersection_it << std::endl;
}

template <class BV, class Succ>
void run_basic(const std::string &path, const std::string &type, const std::vector<uint64_t> &qrandom){
...
int main(int argc, char** argv){
    ...
    util::file::read_from_file(argv[2], random);
    run_basic<hyb_type, typename hyb_type::succ_1_type>(folder, "hyb", random);
    run_basic<rrr_type, typename rrr_type::succ_1_type>(folder, "rrr", random);
    run_basic<sd_type, sd_succ_type>(folder, "sd", random);
    run_basic<oz_type, typename oz_type::succ_1_type>(folder, "oz", random);
